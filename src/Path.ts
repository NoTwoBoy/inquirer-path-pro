...
export default class Path {
  ...
  exists(): boolean {
    try {
      fs.accessSync(this.absolutePath, fs.constants.R_OK);
      return true;
    } catch (error) {
      return false;
    }
  ...
  isDirectory(): boolean {
    return (
      this.relativePath.length === 0 || this.relativePath.endsWith(path.sep)
    );
  }
  ...
  equals(p?: Path): boolean {
    ...
    return (
      this.getCurrentWorkingDirectory() === this.getCurrentWorkingDirectory() &&
