...
export default class PathAutocomplete {
  ...
  nextMatch(forward: boolean = true): void {
    ...
    this.matchIndex =
      position >= 0 ? boundedPosition : this.matches.length + boundedPosition;
  }
  ...
  findMatches(): Path[] | null {
    ...
    const matches = fs
      .readdirSync(directory.getAbsolutePath())
      ...
      .filter(
        (child: Path) => !this.directoryOnly || child.isExistingDirectory()
      )
   ...
   */
  static findCommonMatch(matches: Path[]): Path | null {
    // Sort the paths. The 2 extremes are the most different.
    const sortedName = matches.map((p) => p.getRelativePath()).sort();
